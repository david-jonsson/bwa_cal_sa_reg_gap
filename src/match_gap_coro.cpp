...
void coro_init(int nr_coros, int *next_job, int tid, bwt_t *const bwt, int n_seqs, bwa_seq_t *seqs, const gap_opt_t *opt)
{
    ...
    {
        coro_generators.push_back(reg_gap(next_job, tid, bwt, n_seqs, seqs, opt));
        coro_iterators.push_back(coro_generators[i].begin());
    }
}

bool coro_incr(int coro_idx)
{
    ++coro_iterators[coro_idx];
    return *(coro_iterators[coro_idx]);
}
