...
//#endif
        ...
        //bwt_cal_width2(bwt, p->len, p->seq, w);
        {
            cppcoro::generator<int>
        ...
        if (p->len > opt->seed_len)
        {
            ...
            //bwt_cal_width2(bwt, opt->seed_len, p->seq + (p->len - opt->seed_len), seed_w);
        // core function
...
//inline int bwt_match_exact_alt(const bwt_t *bwt, int len, const ubyte_t *str, bwtint_t *k0, bwtint_t *l0)
//{
//    int i;
//    bwtint_t k, l, ok, ol;
//    k = *k0; l = *l0;
...
//}
//
//inline int bwt_cal_width2(const bwt_t *bwt, int len, const ubyte_t *str, bwt_width_t *width)
//{
//    bwtint_t k, l, ok, ol;
//    int i, bid;
//    bid = 0;
...
//}
//
//bwt_aln1_t *bwt_match_gap2(bwt_t *const bwt, int len, const ubyte_t *seq, bwt_width_t *width,
//                          bwt_width_t *seed_width, const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack)
//{ // $seq is the reverse complement of the input read
//    int best_score = aln_score(opt->max_diff+1, opt->max_gapo+1, opt->max_gape+1, opt);
//    int best_diff = opt->max_diff + 1, max_diff = opt->max_diff;
...
//        for (j = 0; j < p->len; ++j) // we need to complement
//            p->seq[j] = p->seq[j] > 3? 4 : 3 - p->seq[j];
//        p->aln = bwt_match_gap2(bwt, p->len, p->seq, w, p->len <= opt->seed_len? 0 : seed_w, &local_opt, &p->n_aln, stack);
//        //fprintf(stderr, "mm=%lld,ins=%lld,del=%lld,gapo=%lld\n", p->aln->n_mm, p->aln->n_ins, p->aln->n_del, p->aln->n_gapo);
//        // clean up the unused data in the record
//        free(p->name); free(p->seq); free(p->rseq); free(p->qual);
//        p->name = 0; p->seq = p->rseq = p->qual = 0;
